...
class DecodeModel(pl.LightningModule):
    ...
    def beam_search_topk(
        ...
        temperature : float
        k : int
            number of top hypotheses to return per batch item

        Returns
        -------
        List[List[Hypothesis]]: [batch_size, k]
        """
        batch_size = src[0].shape[0] * 2  # mul 2 for bi-direction
        batch_beam_size = batch_size * beam_size
        half_bb_size = batch_beam_size // 2
        ...
        best_scores, best_indices = torch.topk(scores, effective_k, dim=1)
        best_split = best_indices // beam_size
        best_indices = best_indices % beam_size
        batch_indices = torch.arange(
            0, batch_size // 2, dtype=torch.long, device=self.device
        ...
        best_indices = (
            best_split * half_bb_size
            + batch_indices.unsqueeze(1) * beam_size
            + best_indices
        )

        ret: List[List[Hypothesis]] = []
        for bidx in range(batch_size // 2):
            candidates: List[Hypothesis] = []
            for kidx in range(effective_k):
                idx = best_indices[bidx, kidx].item()
                score = best_scores[bidx, kidx].item()
                hpy = Hypothesis(hyps[idx], score, "l2r")
                candidates.append(hpy)

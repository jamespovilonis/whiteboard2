...
var LinesRasterizer = (function () {
   ...
   * @param {Array} strokes - array of stroke objects (from StrokeDataSaver / IdentifyLine)
   *                           each stroke has outlinePoints (array of {x,y}) and color
   * @param {Object} tightBbox - { xMin, yMin, xMax, yMax } in original canvas coords
   * @returns {{ canvasImage: HTMLCanvasElement, dataUrl: string, bbox: Object }}
   */
  function rasterizeLine(strokes, tightBbox) {
    ...
    // resolution as the on-screen rendering.  Outline points are in CSS pixels,
    // so we scale the backing store and keep the drawing surface the same size.
    ...
    // Always render in black (#000000) regardless of the pen colour so that
    // the handwriting-to-LaTeX models see maximum-contrast ink.

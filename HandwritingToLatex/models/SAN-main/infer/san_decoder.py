...
class SAN_decoder(nn.Module):
    ...
    def forward(self, cnn_features, images_mask):
        ...
        if False:
            ...
            result = [['<s>', 0, -1, 'root']]
            word_log_probs = []  # per-step log-probability of each chosen word
            ...
            while len(prediction) < 400:
                ...
                word_step_log_probs = torch.log_softmax(word_prob, dim=1)
                p_word = word
                max_log_prob, word = word_prob.max(1)
                # Track per-step confidence for non-EOS predictions
                if word.item() != 0:
                    word_log_probs.append(word_step_log_probs[0, word.item()].item())
            ...
            # Compute mean per-step confidence (geometric mean: exp(mean log-probs))
            if word_log_probs:
                mean_log_prob = sum(word_log_probs) / len(word_log_probs)
            else:
